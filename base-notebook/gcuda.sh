...
podman build --tls-verify=false -t default-route-openshift-image-registry.apps.ibm-hcs.priv/$YOUR_NAMESPACE/base-notebook-cuda -f Dockerfile.ppc64le.cuda .
#podman build --squash-all --tls-verify=false -t default-route-openshift-image-registry.apps.ibm-hcs.priv/$YOUR_NAMESPACE/base-notebook-cuda -f Dockerfile.ppc64le.cuda .
result=$?
echo Result=$result
if [ $result -eq 0 ]; then
 ...
 echo Pushing
 podman push --tls-verify=false default-route-openshift-image-registry.apps.ibm-hcs.priv/$YOUR_NAMESPACE/base-notebook-cuda
else
